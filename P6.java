...
public class P6 {
	...
	if (args.length != 2) {
	    System.err.println("please supply name of file to be parsed and name of file for MIPS output.");
	    System.exit(-1);
	}
	...
	/* Typecheck */
	if (Errors.fatal == false)
	    ((ProgramNode)root.value).typeCheck();
	
	/* Codegen */
	if (Errors.fatal == false) {
	    Codegen.p = outFile;  // set output file
	    ((ProgramNode)root.value).codeGen();
	    Codegen.p.close(); // close output file
	}
	return;
    }
}
